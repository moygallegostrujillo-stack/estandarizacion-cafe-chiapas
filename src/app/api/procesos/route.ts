...
export async function GET() {
  ...
  const user = session?.user as unknown as { id: string; rol: string; sedeId: string | null };
  if (!user) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const areas = await withUserContext(user.id, user.rol as never, user.sedeId, async (tx) => {
